program bin2hdf5_parallel
    ...
    CALL h5screate_simple_f(rank, HDFdims, filespace_id, HDFerror)

    ! Create the dataset with default properties.
    CALL h5dcreate_f(output_file_id, "IntArray", H5T_NATIVE_DOUBLE, filespace_id, dataset_id, HDFerror)
    CALL h5sclose_f(filespace_id, HDFerror)

    ! Each process defines dataset in memory and writes it to the hyperslab in the file. 
    CALL h5screate_simple_f(rank, HDFcount, memspace_id, HDFerror) 
